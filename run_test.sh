#!/bin/bash
#SBATCH -J RpCtrl-test-s1e2   # 任务名字是 test
#SBATCH -o RpCtrl-test-s1e2   # 输出, 目录./out必须存在，否则无法成功提交job. 也可删除此行由系统自动指定.
...
matlab -nodesktop -nosplash -nodisplay -r "test_KLE_normal"
