function [path_ctrl_IteErr,path_ctrl_IteObj,path_ctrl_OptCtrl,path_ctrl_OptState] = path_ctrl_KLE_coeff_GD_solver(V_basis,T_basis,BN_Dirichlet,beta,train_data_inputs_Npts,step_size_GD,max_ite_GD,train_or_test)
% Discription: distributed elliptic control problem using pathwise control function and gradient descent method
...
%%%%%%%%%%%%% initial force guess %%%%%%%%%%%%%
f_initial = zeros(size(V_basis,2),MC_sample_size); 
b_initial = M * f_initial;

% treat Dirichlet BC
for k = 1 : size(BN_Dirichlet,2)
    if BN_Dirichlet(1,k) == -1
       b_initial(BN_Dirichlet(2,k),:) = 0;
    end    
end
...
parfor p = 1 : MC_sample_size
    ...
    u_initial(:,p) = sparse(A) \ b_initial(:,p);
...
f_cell_initial = cell(1,MC_sample_size);
for k = 1 : MC_sample_size
    f_cell_initial{1,k} = sparse((f_initial(:,k))');
end
diag_f_vector_initial = blkdiag(f_cell_initial{1,:});

Obj_initial = 1/(2*MC_sample_size) * sum(diag(full(diag_u_U_vector_initial * diag_M_matrix * diag_u_U_vector_initial'))) + (beta/2)*(1/MC_sample_size)*sum(diag(full(diag_f_vector_initial * diag_M_matrix * diag_f_vector_initial')));
...
while index_ite < max_ite_GD + 1
    ...
    %%%%%%%%%%%%%% update control %%%%%%%%%%%%%%
    delta_f = - step_size_GD * ( beta * f_old + adj_u );
    f_new = f_old + delta_f;
    ...
    for k = 1 : size(BN_Dirichlet,2)
        if BN_Dirichlet(1,k) == -1
            b_new(BN_Dirichlet(2,k),:) = 0;
        end
    end  
    
    % solve state solu
    parfor k = 1 : MC_sample_size
        state_u_new(:,k) = sparse(stiff_matrices{1,k}) \ b_new(:,k);
    end    
    ...
    f_cell = cell(1,MC_sample_size);
    for k = 1 : MC_sample_size
        f_cell{1,k} = sparse((f_new(:,k))');
    end
    diag_f_vector = blkdiag(f_cell{1,:});
    
    Obj_new = 1/(2*MC_sample_size) * sum(diag(full(diag_u_U_vector * diag_M_matrix * diag_u_U_vector'))) + (beta/2)*(1/MC_sample_size)*sum(diag(full(diag_f_vector * diag_M_matrix * diag_f_vector')));

    %%%%%%%%%%%%%% calculate err %%%%%%%%%%%%%%
    relative_err = abs(Obj_new - Obj_old)/Obj_old;
    gradient_err = 0;
    for m = 1 : MC_sample_size
        gradient_err = gradient_err + sqrt( (beta*f_old(:,m)+adj_u(:,m))' * M * (beta*f_old(:,m)+adj_u(:,m)) );
    end
    gradient_err = gradient_err/MC_sample_size;
    
    %%%%%%%%%%%%%% save results %%%%%%%%%%%%%%
    path_ctrl_IteErr(:,index_ite) = [index_ite;relative_err;gradient_err];
    path_ctrl_IteObj(:,index_ite) = [index_ite;Obj_old];
    index_ite = index_ite + 1;
    
end

path_ctrl_OptCtrl = f_new;
path_ctrl_OptState = state_u_new;

%% save results
if train_or_test == 1 % for training
    
    save('Figures/fig_Traindata_RobDetCtrl_vs_PathCtrl/Fig_Traindata_PathCtrl_KLE_Normal.mat', 'train_data_inputs_Npts','path_ctrl_OptCtrl','path_ctrl_OptState','path_ctrl_IteErr','path_ctrl_IteObj','V_basis');
    
    save('Figures/fig_Traindata_RobPathCtrl_vs_PathCtrl/Fig_Traindata_PathCtrl_KLE_Normal.mat', 'train_data_inputs_Npts','path_ctrl_OptCtrl','path_ctrl_OptState','path_ctrl_IteErr','path_ctrl_IteObj','V_basis');
